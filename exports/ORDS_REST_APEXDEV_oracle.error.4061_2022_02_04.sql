...
-- Schema: APEXDEV  Date: 金 2月  04 01:06:11 2022 
--
...
begin
    ...
    sys.htp.p(l_stat);
    -- ファンクションの呼び出し。
    update pkg_test set n = my_parent_pkg.count_up(60);
...
begin
    ...
    sys.htp.p(l_stat);
    -- ファンクションの呼び出し。
    update pkg_test set n = my_parent_pkg.count_up(80);
    select n into l_n from pkg_test fetch first 1 rows only;
    sys.htp.p(to_char(l_n));
    -- 1０秒、処理を待つ。
    dbms_session.sleep(10);
...
begin
    ...
    sys.htp.p(l_stat);
    -- ファンクションの呼び出し。
    update pkg_test set n = my_parent_pkg.count_up(70);
    select n into l_n from pkg_test fetch first 1 rows only;
    sys.htp.p(to_char(l_n));
    -- 1０秒、処理を待つ。
    dbms_session.sleep(10);
...
begin
    ...
    sys.htp.p(l_stat);
    -- ファンクションの呼び出し。
    update pkg_test set n = my_parent_pkg.count_up(90);
    select n into l_n from pkg_test fetch first 1 rows only;
    sys.htp.p(to_char(l_n));
    -- 1０秒、処理を待つ。
    dbms_session.sleep(10);
...
begin
    ...
    sys.htp.p(l_stat);
    -- プロシージャの呼び出し。
    my_parent_pkg.count_in(100);
    sys.htp.p(child_pkg.G_TEST);
    -- 1０秒、処理を待つ。
    dbms_session.sleep(10);
    -- もう一度ファンクションの呼び出し。
    -- 引数が同じでもパッケージ変数が違うので値は変わる。
